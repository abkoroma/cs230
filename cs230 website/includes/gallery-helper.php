...
if (isset($_POST['gallery-submit'])) {
    ...
    if ($file_size > 4 *MB) {
        ...
        $destination = '../watches/'.$new_name;

        $sql = "INSERT INTO reviews (title, description, picpath) VALUES (?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);

        if (!$mysqli_sstmt_prepare($stmt, $sql)) {
            header("Location: ../login.php?error=SQLInjection");
            exit();
        }else {
            mysqli_stmt_bind_param($stmt, "sss", $title, $descript, $bio, $new_name);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_store_result($stmt);

            move_uploaded_file($file_temp_name, $destination);
            header("Location: ../admin.php?error=UploadSuccess");
            exit();
        }
    }
